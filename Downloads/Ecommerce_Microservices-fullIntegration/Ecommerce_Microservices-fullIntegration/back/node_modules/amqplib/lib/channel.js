    ...
    function reply(err, f) {
      if (err === null) {
        if (f.id === expect) {
          ...
          var e = new Error(
            fmt("Expected %s; got %s", expectedName, inspect(f, false))
          );
          self.closeWithError(
            f.id,
            fmt("Expected %s; got %s", expectedName, methodName(f.id)),
...
function invalidateSend(ch, msg, stack) {
  ch.sendImmediately = ch.sendOrEnqueue = ch.sendMessage = invalidOp(
    msg,
    stack
  );
}
