...
export class AdminDashboardComponent implements OnInit {
  ...
  selectedPromoCode: PromoCode = { code: '', discountType: 'PERCENTAGE', value: 0, expiryDate: '', maxUsages: 100, active: true };
  ...
  onImageFileSelected(event: Event, mode: 'add' | 'edit'): void {
    ...
    this.articleService.uploadImage(file).subscribe({
      ...
      error: (err) => {
        ...
        this.isUploadingImage = false;
        alert('Erreur lors du téléchargement de l\'image. Vérifiez que le serveur est bien démarré.');
      }
    });
  }
  ...
  getFilteredOrders(): AdminOrder[] {
    ...
    return this.orders.filter(o => {
      return (
        ...
        String(o.amount ?? '').toLowerCase().includes(term) ||
  ...
  getOrderStats() {
    ...
    const confirmed = list.filter(o => (o.status || '').toUpperCase() === 'CONFIRMED').length;
    const shipped = list.filter(o => (o.status || '').toUpperCase() === 'SHIPPED').length;
    const delivered = list.filter(o => (o.status || '').toUpperCase() === 'DELIVERED').length;
    const cancelled = list.filter(o => (o.status || '').toUpperCase() === 'CANCELLED').length;
    const revenue = list
  ...
  updateOrderStatus(): void {
    ...
    const newStatus = this.orderService.normalizeBackendStatus(this.orderForm.value.status);
  ...
  selectPromoForEdit(promo: PromoCode): void {
    ...
    if (this.selectedPromoCode.expiryDate) {
      this.selectedPromoCode.expiryDate = new Date(this.selectedPromoCode.expiryDate).toISOString().slice(0, 16);
    }
  }

  savePromo(): void {
    if (this.isEditingPromo && this.selectedPromoCode.id) {
      this.promoCodeService.updateCode(this.selectedPromoCode.id, this.selectedPromoCode).subscribe({
        next: () => {
  ...
  togglePromoActive(id: number): void {
    this.promoCodeService.toggleActive(id).subscribe({
      ...
      error: () => this.promoError = 'Erreur lors de l\'activation/désactivation.'
    });
  }

  resetPromoForm(): void {
    this.selectedPromoCode = { code: '', discountType: 'PERCENTAGE', value: 0, expiryDate: '', maxUsages: 100, active: true };
