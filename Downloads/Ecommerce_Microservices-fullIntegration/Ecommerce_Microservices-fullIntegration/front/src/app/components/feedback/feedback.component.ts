...
export class FeedbackComponent implements OnInit, OnChanges {
  ...
  submitFeedback(): void {
    if (this.feedbackForm.invalid || this.feedbackForm.value.rating === 0) {
      this.errorMessage =
        'Veuillez remplir tous les champs et donner une note.';
      return;
    }
    ...
    this.feedbackService.create(newFeedback).subscribe({
      ...
      error: err => {
        this.errorMessage =
          err.error?.error ||
          'Erreur lors de la soumission. Vous avez peut-être déjà soumis un avis.';
        this.isSubmitting = false;
      }
    });
  }
  ...
  get currentRating(): number {
    return this.feedbackForm.value.rating || 0;
  }
  get currentEditRating(): number {
    return this.editForm.value.rating || 0;
  }
  get commentLength(): number {
    return this.feedbackForm.value.comment?.length || 0;
  }
}
