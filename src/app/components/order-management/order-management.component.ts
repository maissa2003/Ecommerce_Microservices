...
import { OrderService, Order as ApiOrder, OrderItem } from '../../services/order.service';
...
export class OrderManagementComponent implements OnInit {
  ...
  createOrder(): void {
    if (this.orderForm.valid && this.currentOrderItems.length > 0) {
      const total = this.currentOrderItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
  ...
  updateOrder(): void {
    if (this.orderForm.valid && this.selectedOrder && this.currentOrderItems.length > 0) {
      const total = this.currentOrderItems.reduce((sum, item) => sum + item.price * item.quantity, 0);
      ...
      if (index !== -1) {
        ...
        if (this.selectedOrder.apiId) {
          this.orderService.updateStatus(this.selectedOrder.apiId, this.orderForm.value.status).subscribe({
