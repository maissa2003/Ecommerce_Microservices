...
export interface RegisterRequest { nom: string; email: string; password: string; }
export interface Utilisateur { id: number; nom: string; email: string; role: string; }
export interface AuthResponse { token: string; user: Utilisateur; }
export interface ProfileResponse { nom: string; email: string; }

@Injectable({ providedIn: 'root' })
export class AuthService {
  private base      = 'http://localhost:8888/api/auth';
  private usersBase = 'http://localhost:8888/api/users';
  ...
  register(user: RegisterRequest): Observable<AuthResponse> {
    return this.http.post<AuthResponse>(`${this.base}/register`, user).pipe(
      tap(response => this.saveSession(response))
    );
  }
  ...
  resetPassword(email: string, otp: string, newPassword: string): Observable<any> {
    return this.http.post(`${this.base}/reset-password`, { email, otp, newPassword }, { responseType: 'text' });
  }

  updateMyProfile(nom: string, email: string): Observable<ProfileResponse> {
    return this.http.put<ProfileResponse>(`${this.usersBase}/me/profile`, { nom, email }).pipe(
      tap(response => {
