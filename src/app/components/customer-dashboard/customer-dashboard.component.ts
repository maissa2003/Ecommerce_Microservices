...
export class CustomerDashboardComponent implements OnInit {
  ...
  loadStatistics(): void {
    ...
    this.activeOrders = this.orders.filter(o => o.status === 'processing' || o.status === 'shipped').length;
  }
  ...
  openAddAddressModal(): void {
    this.addressForm.reset({ type: 'home', country: 'France', isDefault: false });
  ...
  updateAddress(): void {
    if (this.addressForm.valid && this.selectedAddress) {
      ...
      if (index !== -1) {
        const updatedAddress = { ...this.selectedAddress, ...this.addressForm.value };
  ...
  updatePaymentMethod(): void {
    if (this.selectedPayment) {
      const index = this.paymentMethods.findIndex(p => p.id === this.selectedPayment!.id);
      if (index !== -1) {
